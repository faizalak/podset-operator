...
import (
	...
	appv1alpha1 "github.com/faizalak/podset-operator/api/v1alpha1"
)
...
func (r *PodSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	//Scaling down pods if the count is greater than replicas needed
	numAvailable := int32(len(available))
	if numAvailable > podSet.Spec.Replica {
		r.Log.Info("Scaling down pods", "Currently available", numAvailable, "#1 Required replicas ", podSet.Spec.Replica)
		diff := numAvailable - 1
	...
	//Scaling up pods if the count is lesser than replicas needed
	if numAvailable < podSet.Spec.Replica {
		r.Log.Info("Scaling up pods", "Currently available ", int32(numAvailable), "#2 Required replicas ", podSet.Spec.Replica)
